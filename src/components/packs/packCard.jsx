                    ...
                    <div className='flex md:flex-row flex-col justify-start gap-5 items-start'>
                        {
                            pack.posts.length > 0 && pack.posts.map(trip => {
                                ...
                                return  <div onClick={() => router.push(pathname + "event" + "?" + createQueryString("title", trip.data.title) + "&" + createQueryString("description", trip.data.description) + "&" + createQueryString("price", trip.data.price) + "&" + createQueryString("endingTime", trip.data.endingTime) + "&" + createQueryString("startingTime", trip.data.startingTime) + "&" + createQueryString("image1", `${config.baseUrl}${trip.pictures[0].value}`)  + "&" + createQueryString("image2", `${config.baseUrl}${trip.pictures[1].value}`)  + "&" + createQueryString("image3", `${config.baseUrl}${trip.pictures[2].value}`))} className='flex w-full rounded-2xl gap-5 justify-between h-36 px-5 items-center bg-[#221E4D]/60'>
